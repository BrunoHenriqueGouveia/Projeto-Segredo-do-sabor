...
export default function Reserva({ produtosReservados, excluirProduto, limparProdutosReservados }) {
  ...
  const atualizarTotalProduto = useCallback((id, total) => {
    setTotaisProdutos((prevTotais) => ({
      ...prevTotais,  // Mantém os valores anteriores
      [id]: total,    // Atualiza apenas o produto específico
    }));
  }, []);
  ...
  const finalizarReserva = async () => {
    ...
    const produtos = produtosReservados.map(produto => ({
      id: produto.id,
      nome: produto.nome,
      preco: produto.preco,
    ...
    const produtosComQuantidade = produtosReservados.map(produto => ({
      id: produto.id,
      quantidade: quantidadesUnitario[produto.id] || 1 
    }));


    const cliente = { nome, email, telefone };
    const reserva = { data, horario, pontoEntrega, turno, totalGeral, status, pagamento, produtos: { produtosReservados: produtos }, produtosComQuantidade };
    ...
    try {
      ...
      // Limpa os produtos reservados
      limparProdutosReservados();
